...
dbConn.query("INSERT INTO job set ?", newEmp, function (err, res) {
if(err) {
...
Job.findByTitle = function (title, result) {
dbConn.query("Select * from employees where title = ? ", id, function (err, res) {
if(err) {
  console.log("error: ", err);
  result(err, null);
}
...
dbConn.query("Select * from job", function (err, res) {
if(err) {
  console.log("error: ", err);
  result(null, err);
}
